...
def draw(processed_exp):
    for i in range(len(g_savenames)):
        ...
        first_tick_locs = ind+width*len(g_systems)/2
        ax.set_ylabel('Throughput (Mtxns/sec)')
        ax.set_xticks(first_tick_locs)
        ax.set_xticklabels(wl_display_name(wl) for wl in g_wls)

        plot_helper.second_xtick_labels(fig, ax, first_tick_locs, ['medium contention', 'high contention'])
